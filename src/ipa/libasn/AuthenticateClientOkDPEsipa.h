/*
 * Generated by asn1c-0.9.29 (http://lionet.info/asn1c)
 * From ASN.1 module "SGP32Definitions"
 * 	found in "../../../asn1/SGP32Definitions.asn"
 * 	`asn1c -fcompound-names -no-gen-example -no-gen-OER -no-gen-PER -fno-constraints`
 */

#ifndef	_AuthenticateClientOkDPEsipa_H_
#define	_AuthenticateClientOkDPEsipa_H_


#include <asn_application.h>

/* Including external dependencies */
#include "TransactionId.h"
#include "SmdpSigned2.h"
#include <OCTET_STRING.h>
#include "Certificate.h"
#include "Octet32.h"
#include <constr_SEQUENCE.h>

#ifdef __cplusplus
extern "C" {
#endif

/* Forward declarations */
struct StoreMetadataRequest;

/* AuthenticateClientOkDPEsipa */
typedef struct AuthenticateClientOkDPEsipa {
	TransactionId_t	*transactionId	/* OPTIONAL */;
	struct StoreMetadataRequest	*profileMetaData	/* OPTIONAL */;
	SmdpSigned2_t	 smdpSigned2;
	OCTET_STRING_t	 smdpSignature2;
	Certificate_t	 smdpCertificate;
	Octet32_t	*hashCc	/* OPTIONAL */;
	/*
	 * This type is extensible,
	 * possible extensions are below.
	 */
	
	/* Context for parsing across buffer boundaries */
	asn_struct_ctx_t _asn_ctx;
} AuthenticateClientOkDPEsipa_t;

/* Implementation */
extern asn_TYPE_descriptor_t asn_DEF_AuthenticateClientOkDPEsipa;
extern asn_SEQUENCE_specifics_t asn_SPC_AuthenticateClientOkDPEsipa_specs_1;
extern asn_TYPE_member_t asn_MBR_AuthenticateClientOkDPEsipa_1[6];

#ifdef __cplusplus
}
#endif

/* Referred external types */
#include "StoreMetadataRequest.h"

#endif	/* _AuthenticateClientOkDPEsipa_H_ */
#include <asn_internal.h>
